...
def main(args):
    ...
    # System prompt
    system_prompt = f"<|system|> You are an AI assistant. Help the user as much as possible."
    # Calculate the tokens for eos_token, assis_token, prompter_token, system_prompt and spaces
    fixed_token_count = count_tokens(eos_token) * 3 + count_tokens(assis_token) + count_tokens(prompter_token) + count_tokens(system_prompt) + 8

    MAX_TOKENS -= fixed_token_count
    ...
    # Generate examples with tokenized count
    def generate_examples(messages):
        ...
        for message in messages:
            ...
            text = message['text']
            current_tokens = [f"{author}: {text}"]
            current_token_count = sum(count_tokens(t) for t in current_tokens)

            if token_count + current_token_count <= MAX_TOKENS:
                if author != 'ChatGPT':
                    tokens.extend(current_tokens)
                    token_count += current_token_count
            else:
                while token_count + current_token_count > MAX_TOKENS:
                    if len(tokens) > 0:
            ...
            if author == 'ChatGPT':
                formatted_example = f"{system_prompt} {eos_token} {prompter_token} {' '.join(tokens)} {eos_token} {assis_token} {text} {eos_token}"
    ...
    dataset = Dataset.from_dict({"content": ds})
